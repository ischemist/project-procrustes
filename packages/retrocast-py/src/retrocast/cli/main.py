...
def _score_one(model_name: str, benchmark_name: str, paths: dict[str, Path], args: argparse.Namespace) -> None:
    ...
    evaluation, output_label, stock_paths = _native_cli_call(
        "score",
        native_score_project_files,
        candidates_path,
        task_path,
        paths["stocks"],
...
def _native_cli_call(action: str, operation: Callable[..., T], /, *args: Any, **kwargs: Any) -> T:
    """Translate extension failures into the CLI's stable domain errors."""
    try:
        return operation(*args, **kwargs)
    except FileNotFoundError as exc:
        raise ArtifactNotFoundError(
            f"Native {action} failed: {exc}",
            code="io.native_not_found",
            context={"action": action, "error": str(exc)},
        ) from exc
    except OSError as exc:
        raise RetroCastIOError(
            f"Native {action} failed: {exc}",
            code="io.native_failure",
            context={"action": action, "error": str(exc)},
        ) from exc
    except ValueError as exc:
        raise InputError(
            f"Native {action} rejected its input: {exc}",
            code="input.native_invalid",
            context={"action": action, "error": str(exc)},
        ) from exc
    except RuntimeError as exc:
        raise WorkflowError(
            f"Native {action} failed: {exc}",
            code="workflow.native_failure",
            context={"action": action, "error": str(exc)},
        ) from exc
