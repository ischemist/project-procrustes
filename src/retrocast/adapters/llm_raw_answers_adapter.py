...
class LlmRawAnswersAdapter(BaseAdapter):
    ...
    def cast(
        ...
        try:
            ...
            logger.warning(f"  - data for target '{target.id}' failed llm-raw-answers schema validation. error: {e}")
            return
        ...
        for rank, record in enumerate(validated.root, start=1):
            try:
                ...
                logger.warning(f"  - completion #{rank} for '{target.id}' failed transformation: {e}")
                continue
    ...
    def _transform(
        ...
        if not precursor_map:
            raise AdapterLogicError("no synthesis steps found in completion")
        ...
        if expected_target not in precursor_map:
            raise AdapterLogicError(
                f"mismatched smiles for target {target.id}: expected canonical '{expected_target}' "
                f"is not produced by any synthesis step in this completion"
            )
