...
```mermaid
graph LR
    A[Model Output<br/><i>Native Format</i>] --> B[Adapter<br/><i>Translation Layer</i>]
    B --> C[Route Objects<br/><i>Canonical Schema</i>]
    C --> D[Evaluation Pipeline<br/><i>Metrics & Analysis</i>]
...
!!! success "Why this matters"
    ...
    This architecture ensures that metrics like stock-termination rate and route depth are calculated identically for every model.
...
Evaluation artifacts refer back to route reactions with deterministic route-local path ids such as `rc:r:_` and `rc:r:1.0`. See [Route Node IDs](developers/route-node-ids.md) for the grammar and examples.
...
- `CandidateRecord`: one raw rank slot after adaptation accounting. It contains either a canonical route or a typed adapter failure.
- `ScoredCandidate`: one ranked candidate plus evaluation annotations: tier validity, scope constraint results, and acceptable-route matching.
...
Ingest writes candidate records by default for target-keyed raw output. Flat provider output remains route-only until target-local candidate slots are defined. Standalone `adapt --preserve-failed-candidates` writes the candidate artifact named by `--output`.

- **Candidate artifact:** `3-processed/<benchmark>/<model>/candidates.json.gz`
- **Shape:** `CandidateRecordsArtifact`
- **Payload:** candidate audit metadata plus `dict[target_id, list[CandidateRecord]]`

`routes.json.gz` is the route-only artifact used by ordinary scoring. `candidates.json.gz` preserves failed rank slots for candidate-denominator metrics.
...
For ordinary route-only scoring, `EvaluationResults.metadata["scoring_denominator"]` records `type="route_only"`. Candidate-preserving scoring records the denominator from `candidates.json.gz`.
...
| `3-processed` | `candidates.json.gz` | `CandidateRecordsArtifact` | Rank-preserving candidate artifact written by default for project ingest. |
