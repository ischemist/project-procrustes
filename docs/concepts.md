...
- `ScoredCandidate`: one ranked route plus evaluation annotations: tier validity, scope constraint results, and acceptable-route matching.
...
`routes.json.gz` is the route-only artifact used by scoring. Failed adaptation slots are not represented as routes.
...
For ordinary route-only scoring, `EvaluationResults.metadata["scoring_denominator"]` records `type="route_only"`. This prevents downstream code from treating saved routes as a complete raw candidate stream.
