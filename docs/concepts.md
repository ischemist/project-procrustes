...
Project ingest writes candidate records by default for target-keyed raw output.
Flat provider output remains route-only until target-local candidate slots are
defined. Standalone `adapt --preserve-failed-candidates` writes the candidate
artifact named by `--output`.
